...
import type { InventoryShiftRow } from './inventoryEngine'
...
export function buildCashClosurePrintDocument(input: {
  ...
  inventoryRows?: InventoryShiftRow[]
}) {
  ...
  const inventory = input.inventoryRows?.length ? `<section><p class="title">ARQUEO DE INVENTARIO</p><table><thead><tr><th>Insumo</th><th>Inicial</th><th>Teórico</th><th>Físico</th><th>Dif.</th></tr></thead><tbody>${input.inventoryRows.map(row => `<tr><td>${escapeHtml(row.name)} (${escapeHtml(row.unit)})${row.count?.note ? `<br><small>${escapeHtml(row.count.note)}</small>` : ''}</td><td>${escapeHtml(row.initial)}</td><td>${escapeHtml(row.theoretical)}</td><td>${row.physical === undefined ? 'Pendiente' : escapeHtml(row.physical)}</td><td>${row.difference === undefined ? '—' : escapeHtml(row.difference)}</td></tr>`).join('')}</tbody></table></section>` : ''
  ...
  <section><p class="title">MOVIMIENTOS MANUALES</p><table><thead><tr><th>Hora</th><th>Tipo</th><th>Concepto</th><th>Medio</th><th class="amount">Monto</th></tr></thead><tbody>${rows}</tbody></table></section>${inventory}<footer>Documento interno de control de caja e inventario</footer></body></html>`
}
