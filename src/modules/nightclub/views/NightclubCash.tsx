...
import { nightclubCashSummary } from '../domain/nightclubAccounts'
...
export function NightclubCash({ data, onStartShift, onCloseShift, onCashMovement }: {
  ...
  const openAccounts = data.accounts.filter(item => item.status !== 'closed')
  ...
  return <div className="space-y-5"><header><h1 className="text-2xl font-black">Caja y turnos</h1><p className="text-sm text-slate-400">Cobros, movimientos y arqueo de la noche.</p></header>
    {!shift || shift.status === 'closed' ? <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5"><p className="mb-3 text-sm">Inicia un turno para abrir cuentas y vender.</p><label className="block text-xs text-slate-400">Fondo inicial / cambio en caja<input aria-label="Fondo inicial" type="number" min="0" value={opening} onChange={event => setOpening(event.target.value)} className="mt-1 block w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-white" /></label><button onClick={() => onStartShift(Number(opening))} className="mt-3 rounded-xl bg-purple-500 px-5 py-3 font-bold text-white">Iniciar turno</button></section> : <>
      <p className="text-xs text-slate-400">Abierto {new Date(shift.openedAt).toLocaleString('es-BO')} · {shift.openedBy}</p>
      <section aria-label="Efectivo esperado en caja" className="flex items-center justify-between gap-4 rounded-2xl border border-purple-400/30 bg-gradient-to-r from-purple-950 to-slate-900 p-5"><div><span className="text-xs font-bold uppercase tracking-wide text-purple-200">Efectivo que debe haber ahora en caja</span><strong className="mt-1 block text-3xl font-black text-white sm:text-4xl">{money(summary.expectedCash)}</strong><p className="mt-2 text-xs text-slate-300">Fondo inicial + ventas en efectivo + entradas en efectivo − salidas en efectivo</p><p className="mt-1 text-xs text-slate-400">QR y tarjeta no se suman al dinero físico.</p></div><Wallet aria-hidden="true" className="hidden h-10 w-10 shrink-0 text-purple-300 sm:block" /></section>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-4"><h2 className="font-bold">Registrar movimiento</h2><div className="mt-3 grid gap-2 sm:grid-cols-4"><select aria-label="Tipo de movimiento" value={type} onChange={event => setType(event.target.value as typeof type)} className="rounded-xl bg-slate-950 p-3"><option value="expense">Salida / gasto</option><option value="income">Entrada</option></select><input aria-label="Concepto" placeholder="Concepto" value={description} onChange={event => setDescription(event.target.value)} className="rounded-xl bg-slate-950 p-3" /><input aria-label="Monto" type="number" min="0.01" step="0.01" placeholder="Monto" value={amount} onChange={event => setAmount(event.target.value)} className="rounded-xl bg-slate-950 p-3" /><select aria-label="Método del movimiento" value={method} onChange={event => setMethod(event.target.value as typeof method)} className="rounded-xl bg-slate-950 p-3"><option value="cash">Efectivo</option><option value="qr">QR</option><option value="card">Tarjeta</option></select></div><button onClick={submitMovement} className="mt-3 rounded-xl bg-purple-500 px-4 py-2 font-bold">Guardar movimiento</button><div className="mt-4 space-y-1 text-sm">{movements.map(item => <p key={item.id} className="flex justify-between border-t border-slate-800 py-2"><span>{item.description} · {item.method}</span><b>{item.type === 'income' ? '+' : '−'}{money(item.amount)}</b></p>)}{!movements.length && <p className="text-slate-400">Sin movimientos manuales.</p>}</div></section>
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-4"><h2 className="font-bold">Arqueo y cierre</h2><p className="mt-1 text-sm text-slate-400">{openAccounts.length ? `${openAccounts.length} cuenta(s) abiertas. Debes cobrarlas antes del cierre.` : 'Todas las cuentas están cerradas.'}</p><div className="mt-3 flex flex-wrap gap-2"><input aria-label="Efectivo contado" type="number" min="0" placeholder="Efectivo contado" value={counted} onChange={event => setCounted(event.target.value)} className="rounded-xl bg-slate-950 p-3" /><button onClick={() => printCash(data)} className="rounded-xl border border-slate-700 px-4 py-2 font-bold">Imprimir movimiento del día</button><button disabled={openAccounts.length > 0 || counted === ''} onClick={() => onCloseShift(Number(counted))} className="rounded-xl bg-purple-500 px-4 py-2 font-bold disabled:opacity-40">Cerrar turno</button></div>{counted !== '' && <p className="mt-2 text-sm">Diferencia: {money(Number(counted) - summary.expectedCash)}</p>}</section>
    </>}
