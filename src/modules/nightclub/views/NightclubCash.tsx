...
export function NightclubCash({ data, onStartShift, onCloseShift, onCashMovement }: {
  ...
  return <div className="space-y-5"><header><h1 className="text-2xl font-black">Caja y turnos</h1><p className="text-sm text-slate-400">Cobros, movimientos y arqueo de la noche.</p></header>
    {!shift || shift.status === 'closed' ? <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5"><p className="mb-3 text-sm">Inicia un turno para abrir cuentas y vender.</p><label className="block text-xs text-slate-400">Fondo inicial / cambio en caja<input aria-label="Fondo inicial" type="number" min="0" value={opening} onChange={event => setOpening(event.target.value)} className="mt-1 block w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-white" /></label><button onClick={() => onStartShift(Number(opening))} className="mt-3 rounded-xl bg-purple-500 px-5 py-3 font-bold text-white">Iniciar turno</button></section> : <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[['Fondo inicial', shift.openingFloat], ['Ventas efectivo', summary.cashSales], ['Ventas QR', summary.qrSales], ['Ventas tarjeta', summary.cardSales], ['Entradas efectivo', summary.cashIncome], ['Salidas efectivo', summary.cashOutflow], ['Total vendido', summary.totalSales], ['Efectivo esperado', summary.expectedCash]].map(([label, value]) => <div key={label} className="rounded-2xl border border-slate-800 bg-slate-900 p-4"><span className="text-xs text-slate-400">{label}</span><strong className="mt-2 block text-xl">{money(Number(value))}</strong></div>)}</div>
