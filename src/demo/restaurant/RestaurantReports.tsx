...
export function RestaurantReports({ orders, shift }: { orders: Order[]; shift: RestaurantShift | null }) {
  ...
  return <div className="space-y-6"><header className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold text-slate-900">Reportes de Salón & Ventas</h1><p className="text-sm text-slate-500">Ventas cobradas y productos del {shift ? 'turno activo' : 'historial disponible'}.</p></div><span className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold"><Calendar size={16} /> {shift ? new Date(shift.openedAt).toLocaleDateString('es-BO') : 'Todos los turnos'}</span></header><div className="grid gap-4 sm:grid-cols-3"><Card label="Total vendido" value={money(summary.totalSales)} /><Card label="Ventas efectivo" value={money(summary.cashSales)} /><Card label="Ventas QR / tarjeta" value={money(summary.qrSales + summary.cardSales)} /></div><section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="mb-4 font-bold">Productos más vendidos</h2>{top.length ? <div className="space-y-3">{top.map(item => <div key={item.name}><div className="flex justify-between gap-2 text-xs font-semibold"><span>{item.name}</span><span>{item.count} · {money(item.total)}</span></div><div className="mt-1 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-teal-600" style={{ width: `${item.count / max * 100}%` }} /></div></div>)}</div> : <p className="text-sm text-slate-500">Aún no hay ventas cobradas para mostrar.</p>}</section></div>
}
