...
const lines=Object.entries(keys).map(([key,name])=>`VITE_FIREBASE_${name}=${JSON.stringify(config[key])}`)
if(typeof config.measurementId==='string'&&config.measurementId.trim()) lines.push(`VITE_FIREBASE_MEASUREMENT_ID=${JSON.stringify(config.measurementId)}`)
fs.writeFileSync('.env.local',lines.join('\n')+'\nVITE_USE_FIREBASE_EMULATOR=false\n')
fs.writeFileSync('.firebaserc',JSON.stringify({projects:{dev:config.projectId}},null,2)+'\n')
