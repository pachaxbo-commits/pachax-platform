...
const ids = {
  ...
  ownerNightclub: `onboarding-nightclub-${suffix}`,
...
try {
  ...
  const nightclubResult = await callOnboarding(await signIn(ids.ownerNightclub), {
    requestId: `nightclub-${suffix}`, companyName: 'Club QA', businessType: 'nightclub_lounge',
  })
