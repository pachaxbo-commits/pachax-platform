...
try {
  ...
  await admin.doc(`tenants/${ids.nightclub}/nightclubProducts/beer`).set({ id: 'beer', tenantId: ids.nightclub, name: 'Cerveza QA', price: 25, active: true, preparationArea: 'Directo', recipe: [{ inventoryId: 'beer-stock', quantity: 1 }] })
  await admin.doc(`tenants/${ids.nightclub}/nightclubInventory/beer-stock`).set({ id: 'beer-stock', tenantId: ids.nightclub, name: 'Cerveza QA', current: 10 })
  ...
  const added = await command(`night-round-${suffix}`, 'addRound', { accountId, items: [{ productId: 'beer', quantity: 2 }] })
  assert(added.response.ok); pass('Function deriva precio y descuenta inventario')
  await command(`night-round-${suffix}`, 'addRound', { accountId, items: [{ productId: 'beer', quantity: 2 }] })
  assert.equal((await admin.doc(`tenants/${ids.nightclub}/nightclubInventory/beer-stock`).get()).data().current, 8); pass('retry de ronda no duplica consumo')
  await command(`night-bill-${suffix}`, 'requestBill', { accountId })
  const payment = await command(`night-payment-${suffix}`, 'recordPayment', { accountId, method: 'cash', amount: 50 })
  assert(payment.response.ok); await command(`night-payment-${suffix}`, 'recordPayment', { accountId, method: 'cash', amount: 50 }); assert.equal((await admin.doc(`tenants/${ids.nightclub}/nightclubPayments/payment_night-payment-${suffix}`).get()).exists, true); assert.equal((await admin.doc(`tenants/${ids.nightclub}/nightclubCashMovements/cash_night-payment-${suffix}`).get()).data().amount, 50); pass('retry de pago conserva un pago, un movimiento de caja y ACK')
  ...
  await assertSucceeds(getDoc(doc(nightclubOwner, 'tenants', ids.nightclub, 'nightclubPayments', `payment_night-payment-${suffix}`))); pass('owner Nightclub lee pago de su sucursal')
  await assertSucceeds(getDoc(doc(nightclubOwner, 'tenants', ids.nightclub, 'nightclubCashMovements', `cash_night-payment-${suffix}`))); pass('owner Nightclub lee movimiento de caja de su sucursal')
